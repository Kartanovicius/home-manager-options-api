...
export const parseConfigurationOptions = async () => {
  ...
  rewriter
    ...
    .on('dl.variablelist dd > table a.filename', {
      ...
      text(text: HTMLRewriterTypes.Text) {
        if (text.text.length === 0) return
        const lastData = data[data.length - 1]
        lastData.declared_by = text.text
          .replace('\n&lt;', '')
          .replace('&gt;\n', '')
      },
    })
  ...
  const _response = rewriter.transform(
    await fetch('https://nix-community.github.io/home-manager/options.xhtml'),
  )
